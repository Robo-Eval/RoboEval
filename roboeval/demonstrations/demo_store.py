...
from roboeval.const import CACHE_PATH, DEMO_RELEASES, DEMO_VERSION
...
from roboeval.demonstrations.demo import Demo, LightweightDemo
...
class DemoStore:
    ...
    def get_demos(
        ...
        indecies: Optional[list[int]] = None,
    ) -> list[Demo]:
    ...
    def load_demo_from_path(self, demo_path: Path, metadata: Metadata, frequency: int = CONTROL_FREQUENCY_MAX) -> Demo:
        """Load a demo from a given path.

        :param demo_path: The path to the demo file.
        :param metadata: The metadata for the target environment (used to create env for observation recreation).
        :param frequency: The target frequency for decimation.

        :return: The loaded demo.
        """
        ...
        # Create environment from the provided metadata (not demo's metadata)
        # This allows using a different observation config than what was recorded
        env = metadata.get_env(frequency)
    ...
    def get_demos_from_folder(
        ...
        demos = [Demo.from_safetensors(file) for file in files]
        ...
        with tqdm(
            ...
            robot = demos[0].metadata.get_robot()
            env = metadata.get_env(frequency)
            for demo in demos:
                demo = DemoConverter.decimate(
                    demo,
                ...
                if metadata.observation_mode != ObservationMode.Lightweight:
                    demo = DemoConverter.create_demo_in_new_env(demo, env)
                new_demos.append(deepcopy(demo))
                # self.cache_demo(demo, frequency)
                pbar.update()
        return new_demos
    ...
    def get_demo_paths_from_folder(self, demos_dir: Path) -> list[Path]:
        """Get demo paths from a folder.

        :param demos_dir: The directory containing the demos.

        :return: List of demo paths.
        """
        
        if not demos_dir.exists():
            return []
        
        return list(demos_dir.glob(f"*{SAFETENSORS_SUFFIX}"))
    # def get_demos_from_folder(self, demos_dir: Path, amount: int) -> list[Demo]:
    #     if not demos_dir.exists():
    #         return []
    #     files = list(demos_dir.glob(f"*{SAFETENSORS_SUFFIX}"))
    #     if amount > len(files):
    ...
    def pull_demos(self):
        ...
        logging.warning(f"Cached demos not found at {self._cache_path}. Downloading from: {url}")
        try:
